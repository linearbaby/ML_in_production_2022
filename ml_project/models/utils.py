...
def get_model(model_config: DictConfig) -> sklearn.base.BaseEstimator:
    ...
    try:
        ...
        log.debug(f"successfully created model object of class{model_config.model_type}")
...
def load_recent_model(model_config: DictConfig):
    ...
    if not os.path.isdir(model_config.model_pkl_path):
        ...
        latest_time = sorted(os.listdir(model_config.model_pkl_path + "/" + latest_date), reverse=True)[0]
        model_path =    model_config.model_pkl_path + "/" +\
